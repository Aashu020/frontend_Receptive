        ...
        >
          ...
          <div className={`relative h-full flex items-center justify-${slide.alignment === 'left' ? 'start' : 'end'} px-4 md:px-16 lg:px-24`}>
            ...
            <div className={`max-w-md md:max-w-xl bg-white/10 md:top-11 2xl:max-w-6xl backdrop-blur-lg rounded-xl p-6 md:p-8 2xl:p-28 shadow-xl ${slide.alignment === 'left' ? 'ml-0 md:ml-10' : 'mr-6 md:mr-10'} relative`}>
              <h1 className="text-2xl md:text-4xl lg:text-5xl  2xl:text-6xl font-bold text-white mb-4 drop-shadow-md">
                {slide.title}
              </h1>
              <p className="text-md md:text-xl  2xl:text-3xl text-white mb-6 drop-shadow-md">
                {slide.description}
              </p>
              <button 
              onClick={() => handleExploreClick(slide.route)} 
              className="text-sm md:text:md 2xl:text-2xl bg-[#D8C287] text-[#0a2d27] px-6 py-3 rounded-lg font-medium hover:bg-[#0a2d27] hover:text-white transition cursor-pointer mb-3 md:mb-8">
                Explore {slide.title}
              </button>
              ...
              <div className="flex justify-between items-center mt-4">
                ...
                >
                  <svg className="md:w-5 md:h-5 h-3 w-3 2xl:w-12 2xl:h-12" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                ...
                <div className="flex space-x-2">
                  {slides.map((_, indicatorIndex) => (
                    <button
                      ...
                      className={`md:w-2.5 md:h-2.5 h-2 w-2 2xl:w-5 2xl:h-5 rounded-full transition-all cursor-pointer ${indicatorIndex === currentSlide ? 'bg-[#D8C287] scale-125' : 'bg-white bg-opacity-70 hover:bg-opacity-100'}`}
