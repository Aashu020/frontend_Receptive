...
function Experience() {
  ...
  const cards = [
    {
      icon: <FaHandshake className="text-3xl md:text-4xl 2xl:text-6xl" />,
    ...
    {
      icon: <FaHistory className="text-3xl md:text-4xl 2xl:text-6xl" />,
    ...
    {
      icon: <FaLightbulb className="text-3xl md:text-4xl 2xl:text-6xl" />,
    ...
    {
      icon: <FaHeadset className="text-3xl md:text-4xl 2xl:text-6xl" />,
    ...
    {
      icon: <FaFileAlt className="text-3xl md:text-4xl 2xl:text-6xl" />,
    ...
    {
      icon: <FaEye className="text-3xl md:text-4xl 2xl:text-6xl" />,
  ...
  return (
    <div className="relative py-5 pt-12 px-6 md:px-12 lg:px-24 bg-[#F8F8F8] overflow-hidden ">
      <h2 className="text-xl md:text-4xl 2xl:text-5xl font-bold text-center mb-3 md:mb-16 text-[#0C3B34] relative z-20">
        HOW RECEPTIVE GROUPS OF COMPANY CAN HELP?
      </h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 relative z-20">
        {cards.map((card, index) => (
          <div 
            key={index}
            className="relative h-100 md:h-96 2xl:h-125 cursor-pointer group"
          ...
          >
            ...
            <div 
              className={`absolute inset-0 rounded-2xl py-10 p-4 md:p-6 2xl:p-8 shadow-lg transition-all duration-500 ease-in-out overflow-hidden flex flex-col items-center justify-center h-full
            ...
            >
              ...
              <div className="flex justify-center mb-4">
                <div className={`p-2 md:p-4 2xl:p-6 rounded-full transition-all duration-300 ${hoveredCard === index ? 'bg-[#D8C287]/20 text-[#D8C287]' : 'bg-[#D8C287]/20 text-[#0C3B34]'}`}>
                  {card.icon}
                </div>
              </div>
              
              <h3 className={`text-lg md:text-xl 2xl:text-3xl font-bold text-center mb-4 2xl:mb-6 transition-colors duration-300 ${hoveredCard === index ? 'text-[#D8C287]' : 'text-[#0C3B34]'}`}>
                {card.title}
              </h3>
              
              <p className={`text-justify text-sm md:text-md 2xl:text-2xl 2xl:text-center 2xl:mb-4 transition-colors duration-300 ${hoveredCard === index ? 'text-white' : 'text-[#333333]'}`}>
                {expandedCard === index ? card.fullContent : card.content}
              </p>
              
              <div className=" md:mt-auto mt-3 md:pt-4 2xl:mb-6 text-center" onClick={() => toggleExpand(index)}>
                <span className={`inline-block px-6 py-2 rounded-lg font-medium 2xl:text-2xl transition-colors duration-300 ${hoveredCard === index ? 'bg-[#D8C287] text-[#0C3B34]' : 'bg-[#0C3B34] text-white'}`}>
